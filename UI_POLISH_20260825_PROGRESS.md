...
- **Stage:** Frontend UI polish deployed — Stage 4 legacy cleanup now unblocked
- **Active backend issue:** `jdwd40/back_coins_x#22` (review-branch-only cleanup)
- **Frontend branch:** `master` (UI branch `v2-ui-polish-20260825` retained remotely)
- **Current frontend production SHA:** `0515afcd688a672bab8616450f50681b7b68c2ae`
...
- **Deployment state:** deployed via current `master` SHA `0515afcd688a672bab8616450f50681b7b68c2ae`; workflow run `32915936070` succeeded; live page/API/browser smoke passed; issue remains open for Pixel 8 Pro acceptance.
...
- Production baseline preserved; Stages 1–3 (#14, #12, #13) are deployed from frontend `master` SHA `0515afcd688a672bab8616450f50681b7b68c2ae`.
- Issue #8 is COMPLETED; #14/#12/#13 are DEPLOYED — AWAITING HUMAN PIXEL ACCEPTANCE and remain open.
- Workflow run `32915936070` succeeded; live HTML/assets, public APIs and browser smoke are verified.
- No production-data changes, migrations, or gameplay-engine changes were made.

## Unresolved problems / safety notes

- Backend `main` has an unrelated untracked `.hermes/` screenshot directory. Preserve it and do not stage it during cleanup.
...
- Legacy cleanup #22 must remain review-branch-only and must not be merged or deployed automatically.

## Deployment state

- Frontend production: `master` SHA `0515afcd688a672bab8616450f50681b7b68c2ae`.
- Workflow: `https://github.com/jdwd40/fcoins_y/actions/runs/32915936070` — successful.
...
- Frontend cleanup has not started; backend main remains `b4d35596aaff848cca00b8a0a6aaa3e676b13c69`.

## Exact next action

Create `v2-legacy-cleanup-20260825` from current backend `main`, preserve the unrelated untracked `.hermes/` screenshot, read live #22/#23 and current backend/frontend reachability, then launch one fresh pinned Kimi K3 audit/implementation task. Commit/push only the safe cleanup branch; do not merge or deploy it.
